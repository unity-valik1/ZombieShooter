...
public class Zombie : MonoBehaviour
{
    ...
    ZombieMovement movement;//ищем ccылку на cвой компонент ZombieMovement у Enemy
    ...
    private void Awake()
    {
        ...
        movement = GetComponent<ZombieMovement>();//получаем ccылка на ZombieMovement у Enemy
    }
    ...
    private void ChangeState(ZombieState newState)
    {
        ...
        {
            case ZombieState.STAND://тогда зомби стоит
                movement.enabled = false;// НЕ ПРИСЛЕДУЕТ ИГРОКА, А СТОИТ НА МЕСТЕ (выключаем движение)
                break;
            case ZombieState.RETURN://тогда зомби возвращается на стартовую позицию
                movement.targetPosiyion = startPosition;//тогда зомби возвращается на стартовую позицию
                movement.enabled = true;// НЕ ПРИСЛЕДУЕТ ИГРОКА, А идет на стартовую позицию
                break;
            case ZombieState.MOVE_TO_PLATER://тогда зомби идет
                movement.enabled = true;//ПРИСЛЕДУЕТ ИГРОКА
                break;
            case ZombieState.ATTACK://тогда зомби атакует
                movement.enabled = false;// ОСТАНАВЛИВАЕТСЯ И АТАКУЕТ
                break;
        }
    ...
    private void DoMove()
    {
        ...
        movement.targetPosiyion = player.transform.position;//тогда зомби идет за игроком
    }
    ...
    private void OnDrawGizmos() //Видить эти радиусы для тестирования и для настройки их
    {
        ...
        Vector3 loodkDirection = -transform.up;
        Vector3 leftViewVector = Quaternion.AngleAxis(viewAngle / 2, Vector3.forward) * loodkDirection;
        Vector3 rightViewVector = Quaternion.AngleAxis(-viewAngle / 2, Vector3.forward) * loodkDirection;
